...
function requestWithPowerShell(url, signal) {
    return new Promise((resolve, reject) => {
        ...
        child.once('close', (code) => {
            ...
            try {
                ...
                if (newline < 0)
                    throw new Error('PowerShell HTTP adapter returned no metadata');
                const meta = JSON.parse(output.slice(0, newline).trim());
...
export function windowsCompatibleFetch(fallback = fetch) {
    if (process.platform !== 'win32' || process.env.X_CLI_NITTER_HTTP === 'fetch')
        return fallback;
...
//# sourceMappingURL=windows-fetch.js.map