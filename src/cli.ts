...
import type { NormalizedPost, SearchProvider } from './core/types.js';
...
function parseArgs(argv: string[]): ParsedArgs {
  const result: ParsedArgs = { command: argv.shift(), positionals: [], options: {} };
...
function optionLimit(args: ParsedArgs, fallback = 10, max = 100): number {
  return cleanLimit(optionString(args, 'limit'), fallback, max);
}

function requireText(args: ParsedArgs, label: string, join = false): string {
  const value = join ? args.positionals.join(' ') : args.positionals[0];
  if (!value) throw new Error(`${label} is required`);
  if (!join && args.positionals.length > 1) throw new Error(`unexpected argument: ${args.positionals.slice(1).join(' ')}`);
  return value;
}
...
function createService(): XService {
  return new XService(
    new FxTwitterProvider({
...
function printPost(post: NormalizedPost): void {
...
function printHuman(command: string, output: Record<string, unknown>): void {
  if (command === 'search' || command === 'timeline' || command === 'media') {
    console.log(`provider: ${output.provider}${output.fallback_used ? ' (fallback)' : ''}`);
    if (output.freshness) console.log(`freshness: ${JSON.stringify(output.freshness)}`);
    for (const post of (output.results as NormalizedPost[] || [])) printPost(post);
    return;
  }
  if (command === 'post' && output.post) {
    printPost(output.post as NormalizedPost);
    return;
  }
  if (command === 'trends') {
    for (const trend of (output.trends as Array<Record<string, unknown>> || [])) console.log(`- ${trend.name || JSON.stringify(trend)}`);
    return;
  }
  console.log(JSON.stringify(output, null, 2));
}

async function execute(args: ParsedArgs): Promise<Record<string, unknown>> {
  const service = createService();
  const command = args.command!;

  if (command === 'search') {
    const feed = optionString(args, 'feed') ?? 'latest';
    if (!['latest', 'top'].includes(feed)) throw new Error('feed must be latest or top');
    const provider = optionString(args, 'provider') ?? 'auto';
    if (!['auto', 'fxtwitter', 'nitter'].includes(provider)) throw new Error('provider must be auto, fxtwitter, or nitter');
    return (await service.searchPosts({
      query: requireText(args, 'query', true),
      limit: optionLimit(args),
      feed: feed as 'latest' | 'top',
  ...
  if (command === 'trends') {
    if (args.positionals.length) throw new Error(`unexpected argument: ${args.positionals.join(' ')}`);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    else printHuman(args.command, output);
  } catch (error) {
