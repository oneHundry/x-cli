...
function classifyStatus(status: number): { kind: ProviderError['kind']; retryable: boolean } {
  ...
  if (status >= 500) return { kind: 'upstream_unavailable', retryable: true };
  return { kind: 'upstream_http_error', retryable: false };
}

function limitResults(result: RawUpstreamResult, count: number): RawUpstreamResult {
...
export class FxTwitterProvider {
  ...
  async search(options: SearchOptions): Promise<RawUpstreamResult> {
  ...
  async getProfileStatuses(options: TimelineOptions): Promise<RawUpstreamResult> {
    const limit = cleanLimit(options.limit);
  ...
  async getProfileMedia(options: TimelineOptions): Promise<RawUpstreamResult> {
    const limit = cleanLimit(options.limit);
    const params = new URLSearchParams({ count: String(limit) });
    optionalParam(params, 'cursor', options.cursor);
    optionalParam(params, 'lang', cleanLang(options.lang));
  ...
  async typeahead(query: string, limit = 10): Promise<RawUpstreamResult> {
    ...
    for (const key of ['users', 'topics', 'events']) {
      ...
      if (Array.isArray(values)) {
        ...
        remaining -= (data[key] as unknown[]).length;
      }
    }
